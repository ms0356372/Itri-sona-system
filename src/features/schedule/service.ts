...
export function preparedCloudRow(sessionId:string,row:PreparedPerson,index:number){const groupCode=groupForSlot(row.slot);if(!groupCode)throw new Error(`「${row.name}」的排程時段無法對應組別：${row.slot}`);return{session_id:sessionId,sequence_no:row.sequence||index+1,employee_no:row.employeeNo,full_name:row.name,gender:row.gender,schedule_slot:row.slot,group_code:groupCode,planned_items:[row.item],note:row.extension?`院內分機：${row.extension}`:''};}
